...
class CombatHUD extends FlxTypedGroup<FlxSprite>
{
	...
	public var spell(default, null):Int;
	...
	public function new()
	{
		...
		enemySprite = new Enemy(background.x + 76, background.y + 16, GOLEM);
		...
		pointer = new FlxSprite(background.x + 10, choices[FIGHT].y + (choices[FIGHT].height / 2) - 8, "assets/images/pointer1.png");
		...
		fledSound = FlxG.sound.load("assets/sounds/fled1.wav");
		hurtSound = FlxG.sound.load("assets/sounds/hurt1.wav");
		loseSound = FlxG.sound.load("assets/sounds/lose1.wav");
		missSound = FlxG.sound.load("assets/sounds/miss1.wav");
		selectSound = FlxG.sound.load("assets/sounds/select1.wav");
		winSound = FlxG.sound.load("assets/sounds/win1.wav");
		combatSound = FlxG.sound.load("assets/sounds/combat1.wav");
	}
	 ...
	 */
	public function initCombat(playerHealth:Int, enemy:Enemy, spell:Int)
	{
		screen.drawFrame();
		...
		//enemyMaxHealth = enemyHealth = if (enemy.type == GOLEM) 2 else 4; // each enemySprite will have health based on their type
	...
	function makeChoice()
	{
		...
		switch (selected) // check which item was selected when the playerSprite picked it
		{
			case FIGHT:
				...
				if (FlxG.random.bool(20*this.spell))
				{
